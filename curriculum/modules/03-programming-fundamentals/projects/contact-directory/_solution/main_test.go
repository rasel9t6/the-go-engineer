...
func TestSearchContactsCaseInsensitive(t *testing.T) {
	dir := make(Directory)
	AddContact(dir, Contact{Name: "Alice", Email: "alice@EXAMPLE.com", Phone: "555-0001"})

	results := SearchContacts(dir, "EXAMPLE")
	if len(results) != 1 {
		t.Fatalf("SearchContacts('EXAMPLE') length = %d; want 1", len(results))
	}
}
...
func TestAddContactEmptyPhone(t *testing.T) {
	dir := make(Directory)
	c := Contact{Name: "Alice", Email: "a@x.com", Phone: ""}
	err := AddContact(dir, c)
	if err == nil {
		t.Error("AddContact with empty phone expected error")
	}
}
