...
// ============================================================================
// Section 11: Concurrency â€” Closing Channels
// Level: Intermediate
// ============================================================================
//
// WHAT YOU'LL LEARN:
...
//   - range over channels â€” automatically stops when channel closes
...
//   4. Closing is optional â€” only needed when receivers must know "we're done"
...
func main() {
	...
	// =====================================================================
	// 2. comma-ok pattern â€” detecting closure manually
	// =====================================================================
	// value, ok := <-ch
	//   ok == true  â†’ value is real data
	//   ok == false â†’ channel is closed, value is zero value
	fmt.Println("2ï¸âƒ£  comma-ok pattern:")
	...
	for {
		...
		if !ok {
			fmt.Println("   Channel closed â€” no more data")
			break
		}
	...
	fmt.Println("3ï¸âƒ£  close() as broadcast signal:")
	...
	for id := 1; id <= 3; id++ {
		...
		go func(workerID int) {
			...
			fmt.Printf("   Worker %d: received shutdown, cleaning up âœ…\n", workerID)
		}(id)
	}

	// Close the channel â€” ALL three workers wake up simultaneously
	...
	fmt.Println("  - ONLY senders close channels â€” NEVER receivers")
	fmt.Println("  - Sending to a closed channel = PANIC (unrecoverable)")
	fmt.Println("\n---------------------------------------------------")
	fmt.Println("ðŸš€ NEXT UP: GC.6 pipeline project")
