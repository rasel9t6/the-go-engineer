...
//
// WHAT YOU'LL LEARN:
//   - I/O Patterns fundamentals and practical application in Go.
//
// WHY THIS MATTERS:
//   - I/O Patterns provides a structured approach to writing clean Go code.
//
// RUN:
...
// Stage 05: Filesystem â€” io.Reader/Writer Patterns
//
...
//
//   - io.Reader and io.Writer â€” Go's most important interfaces
//   - Composing readers and writers (pipes, tee, multi)
//   - Implementing custom readers
//   - io.Copy â€” the universal glue
//
...
func main() {
	...
	// 1. strings.Reader â€” turn a string into an io.Reader
	readerFromString()

	// 2. bytes.Buffer â€” implements BOTH Reader and Writer
	bufferDemo()

	// 3. io.Copy â€” stream data from Reader to Writer
	copyDemo()

	// 4. io.TeeReader â€” read and copy simultaneously
	teeReaderDemo()

	// 5. io.MultiReader â€” concatenate readers
	multiReaderDemo()

	// 6. io.MultiWriter â€” write to multiple destinations at once
	multiWriterDemo()

	// 7. Custom Reader â€” implement the interface yourself
	customReaderDemo()
	fmt.Println("\n---------------------------------------------------")
	fmt.Println("ðŸš€ NEXT UP: FS.7 log search tool")
...
func teeReaderDemo() {
	...
	// Like the Unix `tee` command â€” read and copy simultaneously.
...
func multiReaderDemo() {
	...
	// Reads from each in sequence â€” like cat in Unix.
